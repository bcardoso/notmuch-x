...
This package provides a set of custom extensions for the Emacs [[https://notmuchmail.org/][notmuch]] Emacs interface.
...
(use-package notmuch-x
  ...
  (notmuch-x-search-alt-field-format-mode +1)
  ...
  (defun my/notmuch-tag-archived ()
    ...
    (interactive)
    (notmuch-x-tag-thread '("+archived" "-inbox" "-todo" "-trash" "-unread")
                          :show-next))
